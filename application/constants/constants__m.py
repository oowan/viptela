...
class cConstants:
    ...
    def get_msgs(self):
        return self.__msgs
    def information_about_cConstants_class(self):
        if self.__msgs == True:
            print( "List some Advantages of cConstants : \n" )
            self.pwd_of_this_class()
        elif not self.__msgs == True:
            pass

    def print_out_all_constants(self):
        print("printOutAllConstants" + self.VEDGE_100M)

        #print( [value for name, value in vars(cConstants).iteritems() if not name.startswith('_')])

        print( [v for v in dir(self) if not v.startswith('__')] )
...
def main():
    ...
    res = cConstants(msgs=True)
    res.print_out_all_constants()
